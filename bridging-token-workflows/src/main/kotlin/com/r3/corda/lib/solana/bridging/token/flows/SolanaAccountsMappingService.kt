...
class SolanaAccountsMappingService(appServiceHub: AppServiceHub) : SingletonSerializeAsToken() {
    ...
    init {
        ...
        participants = try {
            (cfg.get("participants") as? Map<String, String>)?.map { (k, v) -> CordaX500Name.parse(k) to Pubkey.fromBase58(v) }
                ?.toMap()
                ?: emptyMap()
        } catch (_: Exception) {
        ...
        mints = try {
            (cfg.get("mints") as? Map<String, String>)?.map { (k, v) -> k to Pubkey.fromBase58(v) }
                ?.toMap()
                ?: emptyMap()
        } catch (_: Exception) {
            emptyMap()
        ...
        mintAuthorities = try {
            (cfg.get("mintAuthorities") as? Map<String, String>)?.map { (k, v) -> k to Pubkey.fromBase58(v) }
                ?.toMap()
                ?: emptyMap()
        } catch (_: Exception) {
            emptyMap()
